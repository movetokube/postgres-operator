...
func Patch(cl client.Client, ctx context.Context, before runtime.Object, after runtime.Object) error {
	resourcePatch := client.MergeFrom(before.DeepCopyObject().(client.Object))
	statusPatch := client.MergeFrom(before.DeepCopyObject().(client.Object))
	...
	if !reflect.DeepEqual(beforeUnstructured, afterUnstructured) {
		err = cl.Patch(ctx, after.DeepCopyObject().(client.Object), resourcePatch)
		if err != nil {
	...
	if (beforeHasStatus || afterHasStatus) && !reflect.DeepEqual(beforeStatus, afterStatus) {
		err = cl.Status().Patch(ctx, after.DeepCopyObject().(client.Object), statusPatch)
		if err != nil {
			errs = append(errs, err)
		}
	}
