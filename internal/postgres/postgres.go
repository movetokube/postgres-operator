...
func NewPG(cfg *utils.Cfg, logger logr.Logger) (PG, error) {
	db, err := GetConnection(cfg.PostgresUser, cfg.PostgresPass, cfg.PostgresHost, cfg.PostgresDefaultDb, cfg.PostgresUriArgs)
	if err != nil {
		log.Fatalf("failed to connect to PostgreSQL server: %s", err.Error())
	}
	logger.Info("connected to postgres server " + cfg.PostgresHost)
	postgres := &pg{
		db:               db,
		log:              logger,
