...
import (
	...
	mockpg "github.com/movetokube/postgres-operator/internal/postgres/mock"
...
var _ = Describe("PostgresReconciler", func() {
	...
	It("should have a working client", func() {
		pg := &v1alpha1.Postgres{
			...
			Status: v1alpha1.PostgresStatus{Succeeded: true, DbName: "superdbname"},
		}
		pg2 := pg.DeepCopy()
		...
		Expect(instance.Status.DbName).To(Equal("superdbname"))
	...
	Describe("Checking deletion logic", func() {
		...
		BeforeEach(func() {
			postgresCR = &v1alpha1.Postgres{
				...
				Status: v1alpha1.PostgresStatus{
					Succeeded: true,
					DbName:    rp.GetPrefixedDbName(name),
		...
		Context("DropOnDelete is enabled", func() {
			...
			BeforeEach(func() {
				...
				expectedDbname := rp.GetPrefixedDbName(name)
				dropGroupRole = pg.EXPECT().DropRole(name+"-owner", "pguser", expectedDbname, gomock.Any())
				dropReaderRole = pg.EXPECT().DropRole(name+"-reader", "pguser", expectedDbname, gomock.Any())
				dropWriterRole = pg.EXPECT().DropRole(name+"-writer", "pguser", expectedDbname, gomock.Any())
				dropDatabase = pg.EXPECT().DropDatabase(expectedDbname, gomock.Any())
				...
				Expect(foundPostgres.Status.DbName).NotTo(BeEmpty())
				Expect(anotherPostgres.Status.DbName).NotTo(BeEmpty())
			})
	...
	Describe("Checking creation logic", func() {
		...
		Context("MasterRole is unset", func() {
			...
			It("should pick a default role name", func() {
				...
				expectedName := expectedDbname + "-group"
				...
				// CreateDB call
				pg.EXPECT().CreateDB(expectedDbname, expectedName).Return(nil)
				// Call Reconcile
				err := runReconcile(rp, ctx, req)
				Expect(err).NotTo(HaveOccurred())
			})
		...
		Context("Correct annotation filter is set", func() {
			...
			It("should create the database", func() {
				...
				pg.EXPECT().CreateDB(expectedDbname, gomock.Any()).Return(nil)
				// Call Reconcile
				err := runReconcile(rp, ctx, req)
				Expect(err).NotTo(HaveOccurred())
			})
		...
		Context("Creation is successful", func() {
			...
			It("should update status", func() {
				expectedRoles := v1alpha1.PostgresRoles{
					Owner:  expectedDbname + "-group",
					Reader: expectedDbname + "-reader",
					Writer: expectedDbname + "-writer",
				}
				// Call Reconcile
				err := runReconcile(rp, ctx, req)
				// No error should be returned
				...
				Expect(foundPostgres.Status.DbName).To(Equal(expectedDbname))
				Expect(foundPostgres.Status.Succeeded).To(BeTrue())
			})
		...
		Context("Creation is not successful", func() {
			...
			BeforeEach(func() {
				...
				pg.EXPECT().CreateDB(expectedDbname, gomock.Any()).Return(fmt.Errorf("Could not create database"))
			})
	...
	Describe("Checking extensions logic", func() {
		...
		Context("Postgres has no extensions", func() {
			...
			It("should not try to create extensions", func() {
				...
				pg.EXPECT().CreateExtension(expectedDbname, gomock.Any(), gomock.Any()).Times(0)
				// Call Reconcile
				err := runReconcile(rp, ctx, req)
				Expect(err).NotTo(HaveOccurred())
			})
		...
		Context("Postgres has extensions", func() {
			...
			Context("Creation is successful", func() {
				...
				BeforeEach(func() {
					...
					pg.EXPECT().CreateExtension(expectedDbname, "pg_stat_statements", gomock.Any()).Return(nil).Times(1)
					pg.EXPECT().CreateExtension(expectedDbname, "hstore", gomock.Any()).Return(nil).Times(1)
				})
			...
			Context("Creation is not successful", func() {
				...
				BeforeEach(func() {
					// Expected method calls
					pg.EXPECT().CreateExtension(expectedDbname, "pg_stat_statements", gomock.Any()).Return(fmt.Errorf("Could not create extension")).Times(1)
					pg.EXPECT().CreateExtension(expectedDbname, "hstore", gomock.Any()).Return(nil).Times(1)
				})
		...
		Context("Subset of extensions already created", func() {
			...
			Context("Creation is successful", func() {
				...
				It("should not recreate existing extension", func() {
					// Expected method calls
					pg.EXPECT().CreateExtension(expectedDbname, "pg_stat_statements", gomock.Any()).Return(nil).Times(1)
					pg.EXPECT().CreateExtension(expectedDbname, "hstore", gomock.Any()).Times(0)
					// Call reconcile
					err := runReconcile(rp, ctx, req)
					Expect(err).NotTo(HaveOccurred())
					// Check updated Postgres
		...
		Context("Postgres has schemas", func() {
			...
			Context("Creation is successful", func() {
				...
				BeforeEach(func() {
					// Expected method calls
					// customers schema
					pg.EXPECT().CreateSchema(expectedDbname, expectedDbname+"-group", "customers", gomock.Any()).Return(nil).Times(1)
					pg.EXPECT().SetSchemaPrivileges(expectedDbname, expectedDbname+"-group", gomock.Any(), "customers", gomock.Any(), gomock.Any()).Return(nil).Times(2)
					// stores schema
					pg.EXPECT().CreateSchema(expectedDbname, expectedDbname+"-group", "stores", gomock.Any()).Return(nil).Times(1)
					pg.EXPECT().SetSchemaPrivileges(expectedDbname, expectedDbname+"-group", gomock.Any(), "stores", gomock.Any(), gomock.Any()).Return(nil).Times(2)
				})
			...
			Context("Creation is not successful", func() {
				...
				BeforeEach(func() {
					...
					pg.EXPECT().CreateSchema(expectedDbname, expectedDbname+"-group", "customers", gomock.Any()).Return(fmt.Errorf("Could not create schema")).Times(1)
					pg.EXPECT().SetSchemaPrivileges(expectedDbname, expectedDbname+"-group", gomock.Any(), "customers", gomock.Any(), gomock.Any()).Return(nil).Times(0)
					// stores schema
					pg.EXPECT().CreateSchema(expectedDbname, expectedDbname+"-group", "stores", gomock.Any()).Return(nil).Times(1)
					pg.EXPECT().SetSchemaPrivileges(expectedDbname, expectedDbname+"-group", gomock.Any(), "stores", gomock.Any(), gomock.Any()).Return(nil).Times(2)
				})
		...
		Context("Subset of schema already created", func() {
			...
			Context("Creation is successful", func() {
				...
				It("should not recreate existing schema", func() {
					...
					pg.EXPECT().CreateSchema(expectedDbname, expectedDbname+"-group", "stores", gomock.Any()).Times(0)
					// Call reconcile
					err := runReconcile(rp, ctx, req)
					Expect(err).NotTo(HaveOccurred())
					// Check updated Postgres
